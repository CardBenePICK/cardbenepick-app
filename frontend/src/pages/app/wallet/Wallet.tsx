...
const Wallet = () => {
  ...
  // ------------------------------------------------------------------
  // [수정] 결제 정보 상태 관리 (외부 변수 수신 or 사용자 입력)
  // ------------------------------------------------------------------
...
useEffect(() => {
    ...
    if (recommendedCardId) {
      ...
      // [추가] 결제 정보가 함께 들어왔다면 모달을 자동으로 열어줄 수 있음
      if (incomingPayment) {
        setIsModalOpen(true);
      }
      
      // navigate(location.pathname, { replace: true, state: {} }); // state 초기화가 필요하다면 주석 해제
    } else {
